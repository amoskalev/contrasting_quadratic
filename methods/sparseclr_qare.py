...
class Model(torch.nn.Module):
    ...
    def return_loss_fn(self, x, t=0.5, eps=1e-8):
        ...
        ####################################################
        # Estimate cosine similarity interAB
        ####################################################
        ...
        # HO term
        ####################################################
        qare_term = self.qap_objective(x_a, x_b)
        ...
        # add margin
        x_m = add_margin(x_ab, margin=1)
        
        # mine positives & negatives
        positives = mine_positives(x_m)
        negatives = mine_negatives(x_m)
        ...
        lap_term = sparse_map.mean()
        
        ####################################################
        # combine
        ####################################################
        total_loss = self.alpha*lap_term + self.beta*qare_term
