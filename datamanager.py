...
class DataManager():
    ...
    def get_train_transforms(self, method, dataset):
        ...
        if(method=="relationnet" or method=="simclr" or method=="simclr_ho" or method=="sparseclr" or method=="sparseclr_ho"):
    ...
    def get_puretrain_loader(self, dataset, data_size, num_workers=8):
        self._check(dataset)        
        if(dataset=="cifar10"): 
            normalize = transforms.Normalize(mean=[0.491, 0.482, 0.447], std=[0.247, 0.243, 0.262])
            test_transform = transforms.Compose([transforms.ToTensor(), normalize])
            test_set = dset.CIFAR10("data", train=True, transform=test_transform, download=True)
        elif(dataset=="stl10"): 
            normalize = transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
            test_transform = transforms.Compose([transforms.ToTensor(), normalize])
            test_set = dset.STL10("data", split="train", transform=test_transform, download=True)
        elif(dataset=="supercifar100"): 
            normalize = transforms.Normalize(mean=[0.507, 0.487, 0.441], std=[0.267, 0.256, 0.276])
            test_transform = transforms.Compose([transforms.ToTensor(), normalize])
            test_set = SuperCIFAR100("data", train=True, transform=test_transform, download=True)
        elif(dataset=="cifar100"): 
            normalize = transforms.Normalize(mean=[0.507, 0.487, 0.441], std=[0.267, 0.256, 0.276])
            test_transform = transforms.Compose([transforms.ToTensor(), normalize])
            test_set = dset.CIFAR100("data", train=True, transform=test_transform, download=True)
        elif(dataset=="tiny"):
            normalize = transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.25, 0.25, 0.25])
            test_transform = transforms.Compose([transforms.ToTensor(), normalize])
            test_set = TinyImageFolder(root="./data/tiny-imagenet-200/train", transform=test_transform)
        elif(dataset=="slim"):
            normalize = transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.25, 0.25, 0.25])
            test_transform = transforms.Compose([transforms.ToTensor(), normalize])
            test_set = SlimImageFolder(root="./data/SlimageNet64/train", transform=test_transform)
        test_loader = torch.utils.data.DataLoader(test_set, batch_size=data_size, shuffle=False, 
                                                   num_workers=num_workers, pin_memory=True)
        return test_loader
